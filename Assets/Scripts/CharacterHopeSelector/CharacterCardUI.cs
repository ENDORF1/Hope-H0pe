    ...
    IPointerEnterHandler, IPointerExitHandler, IPointerClickHandler
{
    ...
    /// <summary>
    /// 黑屏后的拖尾飞升：蓄力阶段不画尾，起飞瞬间拉出光带，飞出后再淡出。
    ...
    public void PlayExitWithTrail(System.Action onComplete = null)
    {
        ...
        seq.OnComplete(() =>
        {
            if (_activeTrail != null && CardTrailRenderer.Instance != null)
                CardTrailRenderer.Instance.FadeOut(_activeTrail, trailFade);
            _activeTrail = null;
            DOVirtual.DelayedCall(trailFade, () => onComplete?.Invoke());
        });
    }
    ...
    void BeginTrail()
    {
        ...
        if (renderer == null || _rect == null) return;

        Color factionColor = GameData.SelectedFaction == TitleScreenManager.Faction.Hope
            ? new Color(0.42f, 0.76f, 1f, 1f)
            : new Color(1f, 0.32f, 0.32f, 1f);
        _activeTrail = renderer.CreateTrail(_rect, factionColor);
    }
