...
public class TitleScreenManager : MonoBehaviour
{
    ...
    /// <summary>全局单例，供其他脚本调用 SwitchFaction 等方法</summary>
    ...
    public class MenuButtonConfig
    {
        ...
        [Tooltip("勾选后，Hope 阵营点击此按钮会触发转场动画（仅开始游戏按钮勾选）")]
    ...
    private void GenerateButtons()
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            SetLayerRecursive(go, LayerMask.NameToLayer("NoReflection")); // 排除出水面倒影
    ...
    public void SwitchFaction()
    {
        ...
        {
            // 单向旋转：每次在当前基础上增加90度
            contentRoot.DORotate(new Vector3(0, 0, 90), halfDuration, RotateMode.LocalAxisAdd)
                ...
                .OnComplete(() =>
                {
                    ...
                    // scale为0的瞬间偷偷reset rotation到-90°（视觉等价于+90°），
                    // 后半段再转90°回到0°，确保每次结束后rotation归零，真标题永远在上
                    contentRoot.localRotation = Quaternion.Euler(0, 0, -90);
                    ...
                    if (factionLabel2 != null)
                    {
                        ...
                        if (glitch != null)
                        {
                            FactionTextConfig txt = CurrentFaction == Faction.Hope ? hopeText : voidText;
                            glitch.Glitch(txt.FactionLabel2);
                        }
                    }
                    ...
                    contentRoot.DOScale(Vector3.one, halfDuration)
                        .SetEase(Ease.OutCubic)
                        .OnComplete(() => _switching = false);
                });

            contentRoot.DOScale(new Vector3(0, 1, 1), halfDuration)
                .SetEase(Ease.InCubic);
        }
        else
        {
    ...
    private void ApplyFaction(Faction faction, bool instant)
    {
        ...
        // 文字颜色（SwitchButtonFX 接管了按钮自身的颜色，此处只更新其他元素）
        ...
        // SwitchButtonFX 接管了 switchFactionButton 的颜色和外观
    ...
    public void StartGame()
    {
        ...
        if (CurrentFaction == Faction.Hope)
        {
            // Hope 转场已在 OnPointerClick 瞬间由 HopeTransition 启动
            // 此处什么都不做，LoadScene 由 HopeTransition 协程负责
            return;
        }
    ...
    /// <summary>所有菜单按钮触发 GlitchInPlace（供 voidGhost 序列调用）</summary>
    public void GlitchAllButtons()
    {
        foreach (var tmp in _buttonTexts)
        {
            if (tmp == null) continue;
            tmp.GetComponent<GlitchText>()?.GlitchInPlace();
        }
    }

    /// <summary>所有菜单按钮持续乱码并变红</summary>
    public void GlitchHoldAllButtons(Color col)
    {
        foreach (var tmp in _buttonTexts)
        {
            if (tmp == null) continue;
            tmp.DOColor(col, 0.3f);
            tmp.GetComponent<GlitchText>()?.GlitchHold();
        }
    }

    /// <summary>所有菜单按钮 Release 乱码并恢复颜色</summary>
    public void ReleaseAllButtons()
    {
        for (int i = 0; i < _buttonTexts.Count; i++)
        {
            var tmp = _buttonTexts[i];
            if (tmp == null) continue;
            tmp.GetComponent<GlitchText>()?.Release();
            // 恢复阵营颜色
            Color c = CurrentFaction == Faction.Hope ? hopeColor : voidColor;
            tmp.DOColor(c, 0.4f);
        }
    }
